          ...
          <div class="mb-3">
            ...
            <select class="livesearch_bagian form-select  @error('bagians') is-invalid @enderror"  name="bagian_id">
              @foreach($bagians as $bagian)
                  <option value="{{$bagian->id}}" @if($dpa->bagian_id === $bagian->id) selected="selected" @endif>{{$bagian->nama_bagian}}</option>
              @endforeach                
            </select>
          </div>
          <div class="mb-3">
            ...
            <select class="livesearch_kegiatan form-select  @error('kegiatans') is-invalid @enderror"  name="kegiatan_id">
              @foreach($kegiatans as $kegiatan)
                  <option value="{{$kegiatan->keg_id}}" @if($kegiatan->keg_id === $sub_kegiatan->kegiatan_id) selected="selected" @endif>{{$kegiatan->kode_kegiatan}} | {{$kegiatan->nama_kegiatan}}</option>
              @endforeach                
            </select>
          </div> 
          <div class="mb-3">
            <label for="exampleFormControlSelect1" class="form-label">Nama Sub kegiatan</label>
            <select class="livesearch_sub_kegiatan form-select  @error('sub_kegiatans') is-invalid @enderror"  name="sub_kegiatan_id">
              @foreach($sub_kegiatan_news as $sub_keg_new)
                  <option value="{{$sub_keg_new->id}}" @if($sub_keg_new->id === $sub_kegiatan->id) selected="selected" @endif>{{$sub_keg_new->kode_sub_kegiatan}} | {{$sub_keg_new->nama_sub_kegiatan}}</option>
              @endforeach                
            </select>
          </div>  
          <div class="mb-3">
            <label for="exampleFormControlSelect1" class="form-label">Nama Rekening</label>
            <select class="livesearch_pilih_rekening form-select  @error('rekenings') is-invalid @enderror"  name="rekening_id">
              @foreach($rekenings as $rekening)
                  <option value="{{$rekening->id}}" @if($dpa->rekening_id === $rekening->id) selected="selected" @endif>{{$rekening->no_rekening}} | {{$rekening->nama_rekening}}</option>
              @endforeach                
            </select>
          </div>
          <div class="mb-3">
            ...
            <select class="livesearch_program form-select  @error('programs') is-invalid @enderror"  name="program_id">
              @foreach($programs as $program)
                  <option value="{{$program->id}}" @if($dpa->program_id === $program->id) selected="selected" @endif>{{$program->nama_program}}</option>
              @endforeach                
            </select>
          </div>
          <hr class="my-4 mx-n4" /> 
